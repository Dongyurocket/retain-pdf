...
- 图书馆手动刷新等待实际加载及排队请求完成，保留当前页和搜索条件；加载失败显示错误并保留已有书籍。
- 修复旧轮询和运行时缓存覆盖较新的失败状态或重试任务的问题。
- OCR 准备、启动及执行异常时正确结束子任务，清理异常进程及读流任务，避免失败后仍被运行状态阻止删除。
- 保留父子任务取消状态；删除冲突区分运行中任务和收藏引用，继续保留普通删除保护，不自动强删。

### 验证

- 前端测试 763/763、Rust API 测试 322/322 通过；类型检查、生产前端构建和桌面前端初始化检查通过。
- 新增整书 OCR 失败后通过真实文档 DELETE 路由删除的回归测试，无需重启或 force 参数。

[v4.3.10]: https://github.com/Dongyurocket/retain-pdf/releases/tag/v4.3.10
